...
import { buildLifecycleSpans } from './build-lifecycle';
 ...
 */
const CONTAINER_KINDS: ReadonlySet<string> = new Set(['entrypoint', 'graphql-field']);

function isContainer(span: RawSpan): boolean {
  // A lifecycle-band span is drawn on its own axis and never parents anything, whatever its kind.
  if (span.lane === 'lifecycle') return false;
...
export function buildTrace(profile: Profile, contributors: readonly TraceContributor[]): void {
  ...
  // Spans opened by hand through `TracerService`. They already carry their real `parentId`,
  // read from the CLS store at the moment they opened, so they need no inference at all.
  for (const span of manualSpansOf(profile)) raw.push(span);

  // Framework phases (guards, validation, controller). They go to the flat lifecycle band rather
  // than into the causal tree: each one overlaps most of the request, so left in the tree they
  // would adopt every span below them and flatten the causality the rest of this function builds.
  for (const span of buildLifecycleSpans(profile)) raw.push(span);
  ...
  for (const span of ordered) {
    ...
    const resolved: TraceSpan = {
      ...
      ...(span.lane && span.lane !== 'trace' ? { lane: span.lane } : {}),
