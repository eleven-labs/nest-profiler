...
With [`@eleven-labs/nest-profiler-routes`](../nest-profiler-routes) installed, the module also contributes a **Discover / Events** view listing every `@OnEvent` subscription discovered across providers and controllers, as `ON <event> → <Provider>.<method>()` — each expanding to the options it registered with (`async`, `prependListener`).
