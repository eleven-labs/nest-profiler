...
describe('EventEmitterCollectorModule', () => {
  describe('forRoot({ enabled: false })', () => {
    ...
    it('leaves the core without an event entrypoint or discover source', async () => {
      ...
      expect(core.getDiscoverSources().some((s) => s.type === 'event')).toBe(false);
  ...
  describe('forRoot()', () => {
    it('provides the collector, the patch, the profiler and the discover source', () => {
      const dynamic = EventEmitterCollectorModule.forRoot();
      expect(dynamic.providers).toEqual(
        expect.arrayContaining([EventEmitterCollector, EventProfilerService, EventDiscoverSource]),
      );
    });
    ...
    it('registers the Events discover source', async () => {
      const { app } = await bootstrap(EventEmitterCollectorModule.forRoot());
      const core = app.get(ProfilerCoreService, { strict: false });

      const source = core.getDiscoverSources().find((s) => s.type === 'event');
