...
- New extensible filter system: filters are `ProfilerListFilter` definitions (key, label, control, `parse`, `matches`) registered via `ProfilerCoreService.registerListFilter()` or the `PROFILER_LIST_FILTERS` multi-token. Each list (HTTP, GraphQL, Commands…) renders its own filter bar with the universal filters plus the filters scoped to its kind; a kind's filters apply only to its own table, and query params are namespaced by section. A filter's `forType` accepts a single kind or a set of kinds, and a `'select'` filter may build its options from the section's profiles via `optionsFor(profiles)` (dynamic options).
- Built-in **universal** filters shown above every list: **min/max duration**, a **With exceptions** checkbox, and a **global search** (entrypoint string fields — URL, command, exchange/routing key… — plus GraphQL operation name). The **status** and **status class** (2xx/3xx/4xx/5xx) filters are scoped to the kinds that carry an HTTP response (HTTP, GraphQL), so they no longer clutter response-less lists such as Commands or RabbitMQ. The HTTP **method** filter is scoped to the HTTP list.
