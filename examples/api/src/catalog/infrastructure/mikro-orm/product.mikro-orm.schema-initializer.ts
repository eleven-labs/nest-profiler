 ...
 * `refresh()` (drop + create); otherwise `synchronize` uses the non-destructive `update()` so a
 * hosted deploy (Vercel + Neon) gets its table created but kept across cold starts.
 */
@Injectable()
export class MikroOrmSchemaInitializer implements OnModuleInit {
  ...
  async onModuleInit(): Promise<void> {
    if (this.config.get<boolean>('database.dropSchema')) {
      ...
      // Keep tables MikroORM doesn't map, such as the TypeORM adapter's `typeorm_products`.
      await this.orm.schema.update({ dropTables: false });
