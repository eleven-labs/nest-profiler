...
describe('Domain events (e2e) — event-emitter collector', () => {
  ...
  it('exposes the @OnEvent subscriptions in the Discover / Events view', async () => {
    const res = await request(server(app)).get('/_profiler').query({ view: 'discover-event' });
    expect(res.status).toBe(200);
    expect(res.text).toContain('listener');
    expect(res.text).toContain('NotificationListener');
    expect(res.text).toContain('product.created');
  });
});
