...
`SQL_ORM` selects which adapter backs the **catalog** context: `mikro-orm` (the default) or `typeorm`. They are mutually exclusive and each maps its own Postgres table — `products` for MikroORM, `typeorm_products` for TypeORM, since their column types and naming differ — so switching `SQL_ORM` against a kept database never makes one ORM reshape the other's rows. The catalog always needs a database. The other contexts stay behind their own flags: the ones whose infrastructure is off are simply not registered, so no connection is attempted and nothing crashes.
