...
| ------------------ | -------------------------- | ----------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `enabled`          | `boolean`                  | `true`      | Synchronous bootstrap decision. When `false`, only the inert no-op layer registers. The library default is `true`. Unused with the dev-dependency install (the entrypoint is the switch). With the `dependencies` install, prefer `ConditionalModule`; this flag is the fallback without `@nestjs/config`, set to `enabled('PROFILER_ENABLED')(process.env)` so it is **off unless `PROFILER_ENABLED=true`**.                                              |
| `isGlobal`         | `boolean`                  | `false`     | Register as a global NestJS module. If you also register `ProfilerNoopModule` (the `ConditionalModule` install, when the app injects `TracerService`), keep `isGlobal` identical on it.                                                                                                                                                                                                                                                                    |
...
| `redaction`        | `ProfilerRedactionOptions` | see notes   | One block covering every capture path: `headers`, `cookies`, `queryParams`, `keys` (object keys in bodies/session/collector payloads), extra value `patterns` and a custom `replacement` (default `[REDACTED]`). Every list is **additive** over the built-ins (`authorization`, `cookie`, `set-cookie`, `x-api-key`, `x-auth-token`, `proxy-authorization`, plus the sensitive query params); `useDefaults: false` turns every built-in list off at once. |

`enabled` and `isGlobal` are synchronous: decided at module-build time, never inside `forRootAsync`'s `useFactory`.

**Less common** (rarely set during initial setup) — `listPageSize`, `maxBodySize`, `traceMinDuration`, `traceIdHeader`, `error`, `runtime`, `sourceContext` / `projectRoot` / `editor`, `version`, `attributes`, `timezone` (IANA zone the UI renders timestamps in; defaults to the host's, worth setting when the app runs in a UTC container), `ignorePaths` / `useDefaultIgnorePaths`, `emitDebugHeaders`, `ignoreRequest`, and `performance.rules`. They exist; reach for the full table in the [configuration docs](https://nest-profiler.eleven-labs.com/docs/packages/nest-profiler/configuration) when you need one. (`ignorePaths` / `ignoreRequest` and `performance.rules` still come up in the production and performance sections below and in the `interpret-performance-tags` skill.)
...
The core never imports `@libsql/client`. Opt in by passing the adapter from the `/sqlite` subpath (optional peer dependency `@libsql/client`, installed alongside with the same flag as the profiler — `-D` for the dev-dependency install). One adapter serves a local file, `:memory:`, or a remote SQLite database:
...
Keep the profiler **off in production by default** — the dev-dependency install never ships it, and the `ConditionalModule` install's `enabled('PROFILER_ENABLED')` (unset ⇒ off) keeps it off. Running it in production therefore requires the `ConditionalModule` install (profiler in `dependencies`). Enabling it in production is a legitimate choice when the API is **not publicly reachable** (internal, behind a VPN) or the user has accepted the exposure — don't refuse it, help them do it with eyes open. The `harden-for-production` skill automates this checklist:

- **Require access control** — the profiler is **open by default**, so in production you MUST provide a `security` strategy (an `authorize` predicate and/or NestJS `guards`; all provided must pass). See "Securing the UI" above.
- **Don't capture bodies** — leave `collectBody: false` (or a small `maxBodySize`), and add only the `redaction` entries / per-collector masks you need on top of the built-ins.
