...
<Callout type="info" title="Want it in devDependencies only?">
  If you never want the profiler in production — zero footprint, never installed there — install it
  with `pnpm add -D @eleven-labs/nest-profiler nestjs-cls` and use the [dev-entry
  split](/docs/packages/nest-profiler/configuration#devdependency-only-the-dev-entry-split) instead
...
Log capture keeps working when off with nothing extra (`createProfilerLogger` is DI-free — see below). Only if a service injects `TracerService` **directly** (`span()`, `captureError()`, attributes…) do you also register `ProfilerNoopModule.forRoot({ isGlobal: true })`, gated on `(env) => !isProfilerEnabled(env)`, so that injection still resolves — see [Enabling and disabling](/docs/packages/nest-profiler/configuration#enabling-and-disabling-the-profiler).
...
Gate each optional collector package the same way. When several profiler modules sit at the root, group them into a single module (the [example app](/docs/example-api) bundles the core plus its global collectors into one `ProfilingModule`) so the composition root keeps a single gate for the active bundle — plus the no-op fallback only if the app injects `TracerService` directly. A top-level `enabled` option is also supported as an alternative, documented once in [Configuration](/docs/packages/nest-profiler/configuration#enabling-and-disabling-the-profiler).
...
@Module({
  imports: [
    ...
    // HttpService through DiscoveryService, so there is nothing else to hand it.
    ConditionalModule.registerWhen(
...
Collectors that read global providers or contribute a global panel should remain in the root module:
...
The Validator panel above needs its validation pipe installed in your bootstrap. This keeps validation independent of the profiler's gate — it runs whether profiling is on or off:
...
```bash
curl http://localhost:3000/api/v1/products       # Catalog (in-memory by default, or SQL with SQL_ORM=typeorm)
