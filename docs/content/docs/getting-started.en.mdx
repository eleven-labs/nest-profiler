...
`ProfilerModule` exports an `IContextAdapter` interface and a `PROFILER_CONTEXT_ADAPTERS` multi-token that let you profile any NestJS execution context beyond HTTP — gRPC, Kafka, WebSockets, and more. `@eleven-labs/nest-profiler-graphql` is the reference implementation. See the [nest-profiler package guide](/docs/packages/nest-profiler#custom-protocol-adapters) for a full adapter example.
