...
description: Color tokens, utility classes and EJS helpers for panel templates.
seo:
  title: Custom collector template reference
  description: 'A complete reference of the color tokens, utility classes and EJS helpers available when building a custom collector panel template, so your panel matches the profiler UI design system out of the box.'
---
...
<Callout type="warn">
  The stylesheet is compiled from the profiler's own templates, so it contains the classes listed
  here and nothing more. A Tailwind utility that appears in no shipped panel — `gap-x-6`,
  `tracking-tighter` — is simply absent from it and does nothing in yours. Build your layout out of
  the classes documented below, or from ones you can see a built-in panel using.
</Callout>
...
| `formatBytes`    | `(bytes: number) → string`                      | Scales a byte count to `B` / `KB` / `MB`, for values spanning several orders  |
| `formatDuration` | `(ms: number) → string`                         | Renders a duration **without its unit** - append `ms` yourself                |
