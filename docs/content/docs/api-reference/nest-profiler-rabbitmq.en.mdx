...
Register the module in the dev-only profiling bundle, like every collector — or, behind the [runtime gate](/docs/packages/nest-profiler/configuration#when-production-code-calls-the-profiler-conditionalmodule), wrap it in `ConditionalModule.registerWhen(RabbitMqCollectorModule.forRoot({ … }), isProfilerEnabled)`.
...
Register the module in the dev-only profiling bundle of the application that consumes your messages (the same process that hosts the profiler). Your `RabbitMQModule` stays in your own application module — the collector discovers the `@RabbitSubscribe` handlers wherever they are declared:
...
Drive the options from `ConfigService` (or any provider) with `forRootAsync()` in the bundle — it resolves option _values_ only, not `enabled`, so behind the runtime gate wrap it in `ConditionalModule.registerWhen(...)` as usual (see [Enabling and disabling the profiler](/docs/packages/nest-profiler/configuration#enabling-and-disabling-the-profiler)):
...
If a service injects `TracerService` directly, keeping that code in production requires the profiler as a regular dependency behind the [runtime gate](/docs/packages/nest-profiler/configuration#when-production-code-calls-the-profiler-conditionalmodule), with its `ProfilerNoopModule` fallback.

`@golevelup/nestjs-rabbitmq` and `amqplib` are optional peer dependencies - the ones you already install to talk to the broker, which stay regular dependencies of your app. Neither module needs changes to your `@RabbitSubscribe` handlers or your publishers.
