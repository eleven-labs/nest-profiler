...
Register it in the dev-only profiling bundle, like every collector — or, behind the [runtime gate](/docs/packages/nest-profiler/configuration#when-production-code-calls-the-profiler-conditionalmodule), wrap it in `ConditionalModule.registerWhen(HttpCollectorModule.forRoot({ … }), isProfilerEnabled)`.

## Recorder

### HttpProfilerRecorder

Injectable façade for recording outgoing HTTP requests into the active profile. This is the API to reach for from application code or a custom instrumentation: inject it and call `capture` with the raw request/response material. Injecting it from application code that ships to production requires the profiler as a regular dependency behind the [runtime gate](/docs/packages/nest-profiler/configuration#when-production-code-calls-the-profiler-conditionalmodule); `HttpCollectorModule` is not global, so import it where the recorder is injected. Its `maskHeaders` property exposes the built-in mask list merged with the configured `maskHeaders`.
...
Select the client(s) to instrument via `instrumentations`, importing each adapter from its subpath, in your dev-only profiling bundle:
...
The axios adapter auto-discovers your `HttpService` instances wherever `HttpModule` is imported, so your feature modules keep importing `HttpModule` and injecting `HttpService` as usual — no `axiosRef` wiring. At bootstrap each selected instrumentation installs its hooks and pushes every `HttpRequestEntry` to the current profile via the `HttpProfilerRecorder`. The collector registered by `HttpCollectorModule` renders the panel.
