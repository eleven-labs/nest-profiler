...
Options passed to `HttpCollectorModule.forRoot()`. They extend the shared `HttpCaptureOptions` capture/redaction contract with an `axios` toggle and an `instrumentations` array.
...
The bundled axios adapter, implementing `HttpInstrumentation`. It patches the `axiosRef` supplied through the options (see `forRootAsync`) and records each request via the recorder — it never imports `@nestjs/axios`. It is installed automatically unless `axios: false` is passed; with no `axiosRef` it is a no-op. You rarely reference it directly.
...
**Optional peer dependency:** `axios ^1.0.0` (type-only — the shape of the `axiosRef` you pass). This package never imports `@nestjs/axios`; you supply your `HttpService.axiosRef` via `HttpCollectorModule.forRootAsync()`, so `@nestjs/axios` is your application's dependency, not the profiler's.

## Setup

Hand the collector your `HttpService.axiosRef` through `forRootAsync()` — the package never imports `@nestjs/axios` itself:
