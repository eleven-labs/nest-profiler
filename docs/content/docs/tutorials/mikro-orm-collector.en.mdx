...
Wrap your repository calls in `span()` so the Performance tab's execution trace labels them, and so the MikroORM queries they issue are nested underneath rather than listed flat against the request:
...
Open `/_profiler` and select the **Schemas / MikroORM** view — it has the same shape as every ORM's, one disclosure per entity over a columns table (see the [TypeORM walkthrough](/docs/tutorials/typeorm-collector) for a shot of it).
