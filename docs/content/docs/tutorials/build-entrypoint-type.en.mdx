...
// The kind's identity glyph, shown next to its sidebar item. Reuse the same constant anywhere
// else the protocol is named (a route source's `DiscoverGroup.icon`, a toolbar entry…).
const WEBSOCKET_ICON = `<svg viewBox="0 0 16 16" fill="currentColor">…</svg>`;
...
<div class="rounded-xl border border-line overflow-hidden bg-surface">
  <table class="w-full">
    ...
    <tbody class="divide-y divide-line-subtle">
      ...
      <tr
        data-row-href="<%= profilerPath %>/<%= p.token %>"
        tabindex="0"
        class="group cursor-pointer hover:bg-surface-muted transition-colors"
      >
        <td
          class="py-2.5 px-4 text-foreground-secondary text-xs tabular-nums whitespace-nowrap group-hover:text-nest transition-colors"
        >
          <%= isoDate(p.createdAt) %>
        </td>
...
Every list table follows the same convention: **`Time` then `Duration` open the row**, and the row itself is the link — `data-row-href` on the `<tr>` (plus `tabindex="0"`, since a `<tr>` is not focusable on its own) is what the profiler's client bundle turns into a click-anywhere navigation. No cell owns the link, and the token is not a column: it lives in the URL, in the `X-Debug-Token` header and on the detail page.
...
Register both the context adapter and the entrypoint type in `onModuleInit` (so they run after the profiler core is available), resolving the core leniently so the module degrades gracefully when the profiler is disabled:
...
@Module({
  providers: [WebSocketContextAdapter],
})
export class WebSocketProfilerModule implements OnModuleInit {
  constructor(
    private readonly moduleRef: ModuleRef,
    @Optional() private readonly adapter?: WebSocketContextAdapter,
  ) {}

  onModuleInit(): void {
    if (!this.adapter) return;
    try {
      // strict: false so the core is found even when this module is a sibling of ProfilerModule.
      const core = this.moduleRef.get(ProfilerCoreService, { strict: false });
      core.registerContextAdapter(this.adapter);
      registerWebSocketEntrypoint(core);
    } catch {
...
Register it in the dev-only profiling bundle, next to the core module. Like any collector, a module that imports the profiler follows the dev-only rule when it lives in your application: keep its files out of production code (for example under `src/profiling/`). Your gateways stay in your production modules.

```ts title="profiling/profiling.module.ts"
import { Module } from '@nestjs/common';
import { ProfilerModule } from '@eleven-labs/nest-profiler';
import { WebSocketProfilerModule } from './websocket-profiler.module';

@Module({
  imports: [ProfilerModule.forRoot({ isGlobal: true, collectBody: true }), WebSocketProfilerModule],
})
...
For a non-HTTP context, `ProfilerInterceptor` looks up the adapter registered for the context type. It uses the profile already in CLS, or calls `recoverProfile()` - which here mints one. It then calls `enrichProfile()` (idempotently) to fill `entrypoint.data`, re-establishes the CLS context so the profiler logger (`createProfilerLogger`) and profile-scoped collectors work inside the handler, runs the handler, then collects and persists the profile off the response path. Because the kind was registered with `registerEntrypointType()`, the profiler renders its dedicated list table, **Message** detail tab and its own filter bar (the universal filters plus your `listFilters`) - no core change required.
