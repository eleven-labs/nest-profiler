...
description: Create your own profiler panel with custom data and an EJS template using the @ProfilerCollector() decorator.
seo:
  title: How to build a custom collector
  description: 'A step-by-step guide to building your own profiler panel with custom data and an EJS template using the @ProfilerCollector() decorator, so you can surface exactly the insights your app needs.'
---
...
import { readProfile } from '@eleven-labs/nest-profiler';
...
export class EventPatchService implements OnModuleInit {
  ...
  onModuleInit(): void {
    ...
    this.emitter.emit = function (event: string | symbol, ...args: unknown[]): boolean {
      ...
      // `readProfile` returns undefined outside a profiled execution — no key literal to get
      // wrong, and no try/catch: "nothing is being profiled" is an answer, not an error.
...
<Callout type="info">
  ...
  EJS helper functions. See the [Template reference](/docs/api-reference/template-reference) for the
  complete list.
</Callout>
...
Register it in the dev-only profiling bundle, next to the core module and the official collectors. The collector imports the profiler, so if it lives in your application it follows the same dev-only rule: keep its files out of production code (for example under `src/profiling/`).

```ts title="profiling/profiling.module.ts"
import { Module } from '@nestjs/common';
import { ProfilerModule } from '@eleven-labs/nest-profiler';
import { EventsProfilerModule } from './events-profiler.module';
