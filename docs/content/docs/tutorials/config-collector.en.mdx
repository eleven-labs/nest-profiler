...
```bash
pnpm add @eleven-labs/nest-profiler-config@alpha
```
...
Use `forRootAsync()` to resolve `maskKeys` from `ConfigService` (or any provider), gated per environment with `ConditionalModule` — the recommended way (for the `enabled` flag, see [Enabling and disabling the profiler](/docs/packages/nest-profiler/configuration#enabling-and-disabling-the-profiler)):
