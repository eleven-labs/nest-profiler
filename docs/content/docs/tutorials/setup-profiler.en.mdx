...
> Want the profiler in `devDependencies` only, with no production footprint? Install it with `pnpm add -D @eleven-labs/nest-profiler nestjs-cls` and follow the [dev-entry split](/docs/packages/nest-profiler/configuration#devdependency-only-the-dev-entry-split) instead of the runtime gate used below.
