...
> Want the profiler in `devDependencies` only, with no production footprint? Install it with `pnpm add -D @eleven-labs/nest-profiler@alpha nestjs-cls` and follow the [dev-entry split](/docs/packages/nest-profiler/configuration#devdependency-only-the-dev-entry-split) instead of the runtime gate used below.
...
async function bootstrap() {
  ...
  // createProfilerLogger is DI-free — no TracerService, and a pass-through when the profiler is off.
...
injects `TracerService` directly do you also register `ProfilerNoopModule.forRoot({ isGlobal: true })`
