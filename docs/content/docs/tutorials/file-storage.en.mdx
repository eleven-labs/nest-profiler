...
It is opt-in: `@libsql/client` is an optional peer dependency, so memory/file users pull nothing extra. It is only used by the profiler, so install it as a dev dependency alongside the profiler packages:
...
To store profiles in a remote SQLite database instead — no separate adapter — pass `url` (and an optional `authToken`); it takes precedence over `path`:
...
<Callout type="warn" title="Production warning">
  ...
  type - in production environments. Install it as a dev dependency so production never loads it
  (see [Enabling and disabling the
  profiler](/docs/packages/nest-profiler/configuration#recommended-install-it-as-a-dev-dependency)),
  or keep it off with the [runtime
