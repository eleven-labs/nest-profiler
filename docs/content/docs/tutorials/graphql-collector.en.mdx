...
`GraphQLCollectorModule` registers a `GraphQLContextAdapter` and a `graphql` entrypoint type: for each GraphQL request it recovers the profile the HTTP middleware already created, captures the operation metadata, and flips `entrypoint.type` from `http` to `graphql` so it routes to the dedicated **GraphQL** list table and detail tab. See [How it works](/docs/packages/nest-profiler-graphql#how-it-works) on the package page.
