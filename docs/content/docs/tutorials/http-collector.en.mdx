...
# only if you select the axios adapter — your app owns these (regular dependencies):
pnpm add @nestjs/axios axios
```

## Step 2 - Register the module and select your client(s)

Import each adapter from its subpath and list it in `instrumentations`, in your dev-only profiling bundle. The axios adapter **auto-discovers** every `HttpService` in your app (via `DiscoveryService`), so there is no `axiosRef` to wire — even across several feature modules that each create their own `HttpService`.
...
Your feature module keeps importing `HttpModule` as usual, with no profiler import:
...
Keeping such a call in production code requires installing the profiler as a regular dependency behind the [runtime gate](/docs/packages/nest-profiler/configuration#when-production-code-calls-the-profiler-conditionalmodule), since a dev-dependency install is absent in production; the recorder is exported by `HttpCollectorModule`, which is not global, so import the gated module where the service is declared. To stay dev-only, prefer a reusable `HttpInstrumentation` registered in the bundle (below).
