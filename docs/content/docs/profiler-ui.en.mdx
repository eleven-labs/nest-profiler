...
The **execution timeline** of the spans recorded with `startSpan()` closes the tab, rendered as synchronized bars plus a per-phase table. A profile with no span simply shows no timeline.
...
![Performance tab with duration, CPU time, heap delta, event-loop utilization and the execution timeline of the recorded spans](/screenshots/profiler/performance.png)
