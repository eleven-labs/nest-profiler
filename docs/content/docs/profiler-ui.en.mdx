...
When the response was **streamed** — Server-Sent Events, an LLM token stream, NDJSON, a piped file — the tab adds a `streamed` badge and a **Stream** section describing the delivery: time to first chunk, how long the stream ran, how many chunks (and how far apart), and how many bytes. A stream the client hung up on is flagged `aborted`, and its profile is saved all the same.

This is the one case where the request's duration would otherwise be meaningless. A streaming handler returns in microseconds and the transport keeps writing for seconds, so the profiler closes the profile when the response really ends rather than when the handler returns — which is also what lets work done _during_ the stream reach the collector panels. It needs nothing from the application: the figures come from the transport's own writes.
