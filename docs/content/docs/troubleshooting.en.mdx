...
- A `token` option or the `PROFILER_TOKEN` environment variable is set. Send `Authorization: Bearer <token>` with your request, or unset it for local development. See [Securing the UI](/docs/packages/nest-profiler/configuration#securing-the-ui).
