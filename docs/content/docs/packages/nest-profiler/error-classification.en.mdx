...
### Published AMQP messages

Same idea for the messages your app publishes: each row of the **AMQP** panel is judged on its own. A publish carries no status code, so the default rests on `publish()` having rejected — a message the channel merely buffered is a warning, not a failure.
