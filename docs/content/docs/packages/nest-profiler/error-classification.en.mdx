...
description: Define what a failure means for each entrypoint kind — HTTP status codes, GraphQL error codes, message handlers, command outcomes — and drive the error tag and the Errors filter from your own definition.
seo:
...
| `event`    | The `@OnEvent` handler threw — the synthesised status is deliberately not consulted              | `EventEmitterCollectorModule.forRoot({ error })` |
...
### Published RabbitMQ messages

Same idea for the messages your app publishes: each row of the **RabbitMQ** panel is judged on its own. A publish carries no status code, so the default rests on `publish()` having rejected — a message the channel merely buffered is a warning, not a failure.
