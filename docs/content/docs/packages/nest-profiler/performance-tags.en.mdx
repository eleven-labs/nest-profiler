...
The `error` tag is always `danger` and is not configurable — a failed call is not a tunable performance threshold. A custom rule sets its own severity directly on the tag it emits (see below).
