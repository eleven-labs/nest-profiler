...
description: Detect N+1 queries, slow queries, failed HTTP calls, chatty endpoints and large payloads with the core rule-based tagging engine, and filter profiles by tag.
seo:
  description: 'The nest-profiler core runs a rule-based analysis pass once per profile that tags SQL, Mongo and outgoing HTTP calls with slow / N+1 / error / chatty / large-payload, surfaces them as pills, and makes them filterable on the list page. Add your own rules.'
---
...
The list page shows a **Performance tag** select (Slow / N+1 / Chatty / Large payload) that keeps only the profiles carrying that tag, with the tag pills rendered inline on each row. Failures are not performance issues, so the `error` tag has its own **With errors** checkbox (it replaces the former "With exceptions" filter and is broader — it also covers failed HTTP/query calls, not just unhandled exceptions).
