...
- Pick an enable strategy — `ConditionalModule` gating when `@nestjs/config` is present, otherwise the `enabled` flag (add `ProfilerNoopModule` only if you inject `TracerService` directly).
